...
func (h *HeartbeatV2) run() {
	...
	// failes. It can be removed once we remove the fallback announce operation, since control-stream
	// based heartbeats inherit backoff from the stream handle and don't need special backoff.
	var fallbackBackoffTime time.Time
	...
	var announceWaiters []chan struct{}
...
Outer:
	for {
		...
		if shouldAnnounce {
			if time.Now().After(fallbackBackoffTime) {
				if ok := h.inner.FallbackAnnounce(h.closeContext); ok {
					...
					for _, waiter := range announceWaiters {
						close(waiter)
					}
					announceWaiters = nil
				} else {
					...
					fallbackBackoffTime = time.Now().Add(utils.SeventhJitter(h.fallbackBackoff))
		...
		case sender := <-h.handle.Sender():
			...
			for {
				// inner loop performs announcement via the control stream and exits to the
				// outer loop if the control stream closes.  It is otherwise identical.
				...
				case waiter := <-h.testAnnounce:
					shouldAnnounce = true
					announceWaiters = append(announceWaiters, waiter)
		...
		case ch := <-h.testAnnounce:
			shouldAnnounce = true
			announceWaiters = append(announceWaiters, ch)
		case <-h.closeContext.Done():
			return
		}
	}
